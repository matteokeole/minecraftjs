const
	UIElement = function([w, h], [x, y, z = 0], [tsrc, uv], id) {
		// Width & height in px
		this.width = w * UI_SIZE;
		this.height = h * UI_SIZE;
		this.x = x;
		this.y = y;
		this.z = z;
		this.texture = tsrc;
		...
		this.element.style.cssText = `
			...
			height: ${h}px;
			margin: auto;
			left: ${WINDOW_WIDTH / 2 - (w / 2) + x}px;
			top: ${WINDOW_HEIGHT / 2 - (h / 2) + y}px;
			position: absolute;
		...
		this.setPosition = function(x, y) {
			...
			this.element.style.left = `${WINDOW_WIDTH / 2 - (w / 2) + x}px`;
			this.element.style.top = `${WINDOW_HEIGHT / 2 - (h / 2) + y}px`;
		}
	...
	inventory_bar_selector = new UIElement([12, 12], [inventory_bar_selector_slots[0], (WINDOW_HEIGHT / 2 - (11 / 2) * 4 + 1), 1], ["gui/widgets.png", [0, 11]], "inventory_bar_selector"),
	experience_bar = new UIElement([91, 2.5], [0, (WINDOW_HEIGHT / 2 - (11 / 2) * 4 - 31)],  ["gui/icons.png", [0, 32]], "experience_bar"),
...
for (let i = 0; i < Player.maxHealth; i++) {
	if (i % 2 === 0) {
		let heart_outline = new UIElement([4.5, 4.5], [hearth_slots[i / 2], (WINDOW_HEIGHT / 2 - (11 / 2) * 4 - 48)], ["gui/icons.png", [8, 0]], `heart-${i}`);
	}
}
for (let i = 0; i < Player.maxHunger; i++) {
	if (i % 2 === 0) {
		let hunger_outline = new UIElement([4.5, 4.5], [hunger_slots[i / 2], (WINDOW_HEIGHT / 2 - (11 / 2) * 4 - 48)], ["gui/icons.png", [8, 13.5]], `hunger-${i}`);
	}
}
...
for (let i = 0; i < Player.health; i++) {
	if (i % 2 === 0) {
		let heart_inner = new UIElement([4, 3.5], [hearth_slots[i / 2] + 1, (WINDOW_HEIGHT / 2 - (11 / 2) * 4 - 48)], ["gui/icons.png", [26.5, .5]], `heart-${i}`);
	}
}
...
for (let i = 0; i < Player.hunger; i++) {
	if (i % 2 === 0) {
		let hunger_inner = new UIElement([3.5, 3.5], [hunger_slots[i / 2], (WINDOW_HEIGHT / 2 - (11 / 2) * 4 - 48)], ["gui/icons.png", [26.5, 14]], `hunger-${i}`);
	}
}